...
var insertMention = ({
  ...
  (_a = highlightEl.parentNode) == null ? void 0 : _a.removeChild(highlightEl);
...
var removeTrailingBreaks = (el) => {
  ...
  for (let i = len; i > 0; i--) {
    const node = nodes[i];
    if ((node == null ? void 0 : node.nodeName) === "#text")
      return;
    if ((node == null ? void 0 : node.nodeName) === "BR") {
...
var removeFontTags = (el) => {
  ...
  for (let i = 0; i < nodes.length; i++) {
    ...
    (_a = nodes[i].parentNode) == null ? void 0 : _a.appendChild(textNode);
...
var getCaretPosition = (element) => {
  ...
  if (window.getSelection) {
    var range = (_a = window.getSelection()) == null ? void 0 : _a.getRangeAt(0);
    if (!range)
      return caretOffset;
...
var insertAtCaretPos = (parentEl, insertEl) => {
  ...
  for (let i = 0; i < parentEl.childNodes.length; i++) {
    ...
    if (child.innerText) {
      ...
      charCount += ((_a = child.nodeValue) == null ? void 0 : _a.length) || 0;
    }
  }
  ...
  const beforeNode = document.createTextNode(((_b = anchorNode.nodeValue) == null ? void 0 : _b.substring(0, anchorCaretPos)) || "");
  const afterNode = document.createTextNode(((_c = anchorNode.nodeValue) == null ? void 0 : _c.substring(anchorCaretPos)) || "");
  const nextSibling = anchorNode.nextSibling;
  (_d = anchorNode.parentNode) == null ? void 0 : _d.removeChild(anchorNode);
  if (nextSibling) {
...
var insertAfter = (newNode, existingNode) => {
  var _a;
  (_a = existingNode.parentNode) == null ? void 0 : _a.insertBefore(newNode, existingNode.nextSibling);
};
var autoPositionCaret = (anchorNode) => {
  const selection = window.getSelection();
  const anchor = anchorNode ? anchorNode : selection == null ? void 0 : selection.anchorNode;
  if (!anchor)
    return;
  selection == null ? void 0 : selection.collapse(anchor, anchor.childNodes.length);
};
...
var ReactMentionable = forwardRef((props, ref) => {
  ...
  const selectSuggestion = (suggestion) => {
    ...
    if (!editorRef.current || !highlightEl.current)
      return;
    insertMention({
      mentionClassname: ((_a = mentions.find((m) => m.trigger === currentTrigger.current)) == null ? void 0 : _a.mentionClassname) || "",
  ...
  useLayoutEffect(() => {
    ...
    const keyUpListener = (e) => {
      ...
      if (isMatching.current && key === "Tab" || key === " ") {
        ...
        if (!lastNode)
          return;
        const nodeText = ((_a = lastNode == null ? void 0 : lastNode.nodeValue) == null ? void 0 : _a.replace(currentTrigger.current || "", "").toLowerCase()) || "";
        if (highlightEl.current && (matches.current.length === 1 && isMatching.current) || matches.current.map((m) => m.label).includes(nodeText)) {
          insertMention({
            mentionClassname: ((_b = mentions.find((m) => m.trigger === currentTrigger.current)) == null ? void 0 : _b.mentionClassname) || "",
            trigger: currentTrigger.current || "",
        ...
        if ((lastAnchorChild == null ? void 0 : lastAnchorChild.nodeValue) === "") {
          (_c = lastAnchorChild == null ? void 0 : lastAnchorChild.parentNode) == null ? void 0 : _c.removeChild(lastAnchorChild);
        }
        removeTrailingBreaks(anchorNode);
        removeTrailingBreaks(editorRef.current);
        if (((_d = highlightEl.current) == null ? void 0 : _d.innerText.length) === 1) {
          e.preventDefault();
          setShowSuggestions(false);
          isMatching.current = false;
        }
      } else if (isMatching.current && key !== currentTrigger.current) {
        const inputStr = ((_e = highlightEl.current) == null ? void 0 : _e.innerText) || "";
        ...
        const suggestions2 = (_f = mentions.find((m) => m.trigger === currentTrigger.current)) == null ? void 0 : _f.suggestions;
        if (Array.isArray(mention == null ? void 0 : mention.suggestions)) {
          if (suggestions2) {
            matches.current = suggestions2.filter((suggestion) => regex.test(suggestion.label));
            setSuggestions(matches.current);
          }
        } else {
          mention == null ? void 0 : mention.suggestions(searchStr).then((suggested) => {
    ...
    const keyDownListener = (e) => {
      ...
      if (key === "Enter") {
        ...
        highlightEl.current.className = `${(_a = mentions.find((m) => m.trigger === currentTrigger.current)) == null ? void 0 : _a.highlightClassName}`;
  ...
  return /* @__PURE__ */ React.createElement("div", { className: "react-mentionable" }, /* @__PURE__ */ React.createElement("div", { className: "react-mentionable-editor-container" }, /* @__PURE__ */ React.createElement(
    "div",
    {
  ...
  )), showSuggestions && /* @__PURE__ */ React.createElement("div", { className: `react-mentionable-suggestions ${suggestionsClass}` }, suggestions && suggestions.map((suggestion) => {
    if (renderSuggestion)
